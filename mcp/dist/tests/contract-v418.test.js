...
import { loadContract, loadInputSchemas, requireAgentId, safeErrorPayload, toolInputSchema, toolsForAgent, } from '../server.js';
...
test('tool-input schema registry is exact, closed, and projected for every MCP tool', () => {
    ...
    assert.ok(intake.required.includes('accountable_agent'));
    const decompose = toolInputSchema(schemas, 'task.decompose');
    assert.ok(decompose.required.includes('parent_task_id'));
});
...
test('all ten registered agent processes receive their exact allowlist and cannot select identity from payload', () => {
    ...
    for (const agentId of agents) {
        ...
        assert.deepEqual(toolsForAgent(contract, agentId).map((tool) => tool.name), allowed);
...
//# sourceMappingURL=contract-v418.test.js.map