...
FAIL=0
export QNAP_SCRIPT_ROOT QNAP_APP_ROOT QNAP_MESH_ROOT QNAP_BACKUP_ROOT QNAP_TUNNEL_API_KEY_FILE MESH_UID MESH_GID MESH_COS_SCRIPT
...
if [ -r "$COMPOSE_LIB" ]; then
  ...
  if mesh_resolve_compose; then
    ...
    fail_check "Compose V2 could not be resolved from the QNAP Container Station installation"
  fi
else
...
case "$IP_OWNER" in
  ...
  *) fail_check "192.168.7.60 belongs to another Docker endpoint: $IP_OWNER" ;;
esac
if ping -c 1 -W 1 192.168.7.60 >/dev/null 2>&1; then
  if [ "$IP_OWNER" = "mesh-cos-mcp" ]; then
    ...
    fail_check "192.168.7.60 answered ping without the expected Mesh endpoint; investigate LAN conflict"
  fi
else
...
[ -f "$APP_ROOT/.env" ] && pass ".env exists" || fail_check ".env missing"
[ -f "$APP_ROOT/compose.yaml" ] && pass "compose.yaml exists" || fail_check "compose.yaml missing"
if [ -f "$APP_ROOT/.env" ]; then
  ...
  [ -n "$ACTUAL_MESH_ID" ] && [ "$ACTUAL_MESH_ID" = "${MESH_COS_IMAGE_ID:-}" ] && pass "local Mesh tag resolves to recorded immutable image ID" || fail_check "Mesh image identity mismatch"
  printf '%s' "${TUNNEL_IMAGE:-}" | grep -Eq '^ghcr\.io/openai/tunnel-client@sha256:[0-9a-fA-F]{64}$' && pass "tunnel image uses RepoDigest" || fail_check "tunnel image must use immutable ghcr.io/openai/tunnel-client RepoDigest"
  ...
  if [ -n "${MESH_COS_IMAGE:-}" ] && docker image inspect "$MESH_COS_IMAGE" >/dev/null 2>&1; then
    if mesh_run runtime_permissions runtime-state-access \
      docker run --rm \
        --network none \
        --read-only \
        ...
        --entrypoint /bin/sh "$MESH_COS_IMAGE" \
        -c 'test -r /var/lib/mesh/ledger/taskledger.sqlite3 && test -w /var/lib/mesh/ledger/taskledger.sqlite3'; then
      pass "canonical ledger is read/write for runtime UID/GID"
    else
      fail_check "canonical ledger is not read/write for runtime UID/GID"
    fi
  fi
fi
...
if [ -f "$APP_ROOT/compose.yaml" ] && [ -f "$APP_ROOT/.env" ]; then
  if command -v mesh_compose >/dev/null 2>&1; then
    cd "$APP_ROOT" || fail_check "cannot enter application root for Compose render"
    if mesh_run compose_render compose-config mesh_compose --env-file .env -f compose.yaml config; then
      pass "Compose renders"
    else
      fail_check "Compose render failed"
    fi
    cd "$SCRIPT_ROOT" 2>/dev/null || true
  else
    fail_check "Compose render skipped because Compose V2 was not resolved"
  fi
fi
...
if [ "$FAIL" -ne 0 ]; then
  ...
  mesh_log ERROR preflight_complete "result=FAIL"
  mesh_collect_diagnostics "one or more QNAP host preflight checks failed" || true
  echo "DIAGNOSTIC_LOG=$MESH_COS_LOG_FILE" >&2
  exit 1
fi
mesh_log INFO preflight_complete "result=PASS"
