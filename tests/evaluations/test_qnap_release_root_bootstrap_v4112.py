...
def test_current_runbook_retains_canonical_release_root_contract() -> None:
    ...
    assert "cd /share/Docker/cos-mcp/releases/v4.1.18" not in steps
    assert "mkdir -p /share/Docker/cos-mcp/releases/v4.1.18" not in steps
    assert "cp /share/Docker/mesh-cos-mcp-qnap" not in steps
    assert "chmod 0755" not in steps
    assert "sudo sh ./v4.1.18/mesh-cos-mcp-deploy.sh" in steps
    assert "sudo sh ./v4.1.18/mesh-cos-mcp-preflight.sh" in steps
    assert "sudo sh ./v4.1.18/mesh-cos-mcp-verify.sh" in steps
