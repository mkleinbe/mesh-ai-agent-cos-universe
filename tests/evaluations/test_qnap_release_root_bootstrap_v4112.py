...
def test_current_runbook_retains_canonical_release_root_contract() -> None:
    ...
    assert "cd /share/Docker/cos-mcp/releases/v4.4.1" not in steps
    assert "mkdir -p /share/Docker/cos-mcp/releases/v4.4.1" not in steps
    ...
    assert "sudo sh ./v4.4.1/mesh-cos-mcp-preflight.sh" in steps
    assert "sudo sh ./v4.4.1/mesh-cos-mcp-verify.sh" in steps
