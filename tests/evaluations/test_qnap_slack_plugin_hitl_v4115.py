...
def test_v4115_contract_has_ready_scenarios() -> None:
    feature = (ROOT / "specs/qnap-slack-plugin-hitl-v4.1.15.feature").read_text(
        encoding="utf-8"
    )
    for scenario_id in (
        "QNAP-104",
...
def test_qnap_110_111_promotion_is_snapshot_backed_through_postdeploy_verification() -> None:
    deploy = (ROOT / "deployment/qnap/scripts/mesh-cos-mcp-deploy.sh").read_text(
        encoding="utf-8"
    )
    helper = (ROOT / "deployment/qnap/scripts/mesh-cos-qnap-promotion.sh").read_text(
        encoding="utf-8"
    )
    for token in (
