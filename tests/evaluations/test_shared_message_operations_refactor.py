...
def test_qnap_release_identity_preserves_v4_runtime() -> None:
    ...
    assert "# v4.7.0 Enterprise Consulting Skill Consumption" in release
    assert "# v4.4.0 Authority Closure" in release
    assert "canonical Phase 1 authority/runtime contract **4.0.0**" in release
    assert "production QNAP **4.4.0**" in release
