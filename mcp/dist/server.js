...
export function loadContract() {
    return JSON.parse(fs.readFileSync(path.join(repositoryRoot(), 'chatgpt', 'mcp', 'mesh-cos-mcp.v1.json'), 'utf8'));
}
function schemaTools(payload, expectedVersion) {
...
function deepMergeSchema(base, patch) {
    const merged = { ...base };
...
function applySchemaPatches(raw) {
    ...
    for (const [name, patchValue] of Object.entries(patches)) {
        if (!(name in raw))
            throw new Error(`MCP input-schema patch references unknown tool: ${name}`);
        if (!isRecord(raw[name]) || !isRecord(patchValue)) {
            throw new Error(`Invalid MCP input-schema patch for ${name}`);
        }
        raw[name] = deepMergeSchema(raw[name], patchValue);
    }
}
export function loadInputSchemas(contract = loadContract()) {
    ...
    if (fs.existsSync(extensionPath)) {
        ...
        for (const [name, schema] of Object.entries(extensionTools)) {
            if (name in raw)
                throw new Error(`MCP input-schema extension duplicates tool: ${name}`);
            raw[name] = schema;
        }
    }
...
export function toolInputSchema(schemas, name) {
    const schema = schemas[name];
    if (!schema)
        throw new Error(`Missing MCP input schema for ${name}`);
    return schema;
}
export function requireAgentId(contract, env = process.env) {
    const id = env.MESH_COS_AGENT_ID?.trim();
    if (!id)
        throw new Error('MESH_COS_AGENT_ID is required');
    if (!(id in contract.agent_tool_allowlists)) {
        throw new Error('MESH_COS_AGENT_ID is not a registered Workspace Agent');
    }
    return id;
}
export function deploymentRelease(env = process.env) {
...
export function requireDeploymentRelease(env = process.env) {
    const value = deploymentRelease(env);
    if (!value)
        throw new Error('MESH_COS_DEPLOYMENT_RELEASE is required for remote production runtime');
    return value;
}
export function requireLocalStdioContract(contract) {
    if (contract.transport !== 'LOCAL_STDIO')
        throw new Error('Local entrypoint requires LOCAL_STDIO transport');
}
export function toolsForAgent(contract, agentId) {
...
export function actionSchemaDigest(contract, schemas, agentId) {
    const projection = Object.fromEntries(toolsForAgent(contract, agentId)
        .map(tool => tool.name)
        .sort()
...
export function validateArgumentsSize(value) {
    const args = value ?? {};
...
export function safeErrorPayload(error, requestId) {
    if (error instanceof PythonBridgeError) {
        const payload = { ok: false, request_id: requestId, error: error.category };
        if (error.category === 'validation_failed' && error.details?.length)
...
export function createServer(env = process.env, contract = loadContract()) {
    ...
    server.setRequestHandler('tools/call', async (request) => {
        ...
        try {
            if (!names.has(toolName))
                throw new Error('Tool is not allowed for the bound agent');
