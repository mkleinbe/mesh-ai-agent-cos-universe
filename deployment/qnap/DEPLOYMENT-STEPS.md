...
- extracted release: `/share/Docker/cos-mcp/releases/v4.4.1`
...
Stay in `/share/Docker/cos-mcp/releases`. The ZIP creates `v4.4.1/` during extraction.

## Pre-deploy requirements

- exact v4.4.1 candidate CI is green, including 100% Python coverage, owner execution readiness, QNAP bundle/container provenance, modern MCP transport, and security gates;
...
## Safe v4.4.1 deployment

Place `mesh-cos-mcp-qnap-v4.4.1.zip` and `mesh-cos-mcp-qnap-v4.4.1.zip.sha256` directly in `/share/Docker/cos-mcp/releases`, then run:

```sh
cd /share/Docker/cos-mcp/releases
sha256sum -c mesh-cos-mcp-qnap-v4.4.1.zip.sha256
unzip -oq mesh-cos-mcp-qnap-v4.4.1.zip
sudo sh ./v4.4.1/mesh-cos-mcp-deploy.sh
```
...
```sh
sudo env MESH_COS_FORCE_SLACK_HITL_RECONFIGURE=1 sh ./v4.4.1/mesh-cos-slack-hitl-provision.sh
sudo sh ./v4.4.1/mesh-cos-mcp-deploy.sh
```
...
```sh
sudo sh ./v4.4.1/mesh-cos-tunnel-key-provision.sh
sudo sh ./v4.4.1/mesh-cos-mcp-deploy.sh
```
...
```sh
cd /share/Docker/cos-mcp/releases
sudo sh ./v4.4.1/mesh-cos-mcp-backup.sh manual
sudo sh ./v4.4.1/mesh-cos-mcp-preflight.sh
sudo sh ./v4.4.1/mesh-cos-mcp-verify.sh
```
...
PASS requires active release `4.4.1`, application image `mesh-cos-mcp:qnap-v4.4.1-<source-sha12>`, healthy application/tunnel containers, successful live Slack provider-read/qnet readiness verification, `slack_hitl_ready=true`, and:
...
Then execute `docs/chatgpt-published-app-production-acceptance-qnap-v4.4.1.md`.
...
Rollback uses a complete prior immutable release unit. Do not combine release units. Software rollback must not delete or rewrite canonical PF-057 tasks. If v4.4.1 is rolled back before recovery, leave stranded tasks in their existing state until a corrected transport is authorized.
