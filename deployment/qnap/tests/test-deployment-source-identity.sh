...
grep -Fq -- '--force-recreate mesh-cos-mcp' "$DEPLOY" || { echo 'FAIL deploy does not force-recreate only the application candidate' >&2; exit 1; }
grep -Fq 'compose-tunnel-up mesh_compose' "$DEPLOY" || { echo 'FAIL deploy does not reconcile the tunnel after application recreation' >&2; exit 1; }
if grep -F 'compose-tunnel-up mesh_compose' "$DEPLOY" | grep -Fq -- '--force-recreate'; then echo 'FAIL tunnel reconciliation must not force-recreate the tunnel' >&2; exit 1; fi
