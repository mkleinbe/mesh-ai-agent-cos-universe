...
**Current repository/QNAP deployment target: `v4.2.2 Native Slack HITL Provider Transport Repair`.**
...
The published **Mesh CoS MCP** ChatGPT app is the production ChatGPT surface. The QNAP deployment target is **4.2.2** while the canonical MCP runtime/product version remains **4.0.0**.
...
v4.2.1 added narrow compatibility for Slack provider decision text such as `*APPROVE*`. v4.2.2 repairs the production provider-read transport itself: `conversations.replies` is retrieved with authenticated GET/query parameters rather than the generic POST/JSON path that Slack rejected with `invalid_arguments`. Slack write operations remain POST/JSON. Provider errors fail closed and expose only a sanitized error code.

The provider-verified Slack App ID is `A0B49RNE4K0`. QNAP deployment verification now proves the mounted bot credential can read the governed private channel, detecting missing `groups:history`, stale OAuth authorization, invalid credentials, or channel-access drift before ChatGPT acceptance begins.
