...
def _pending() -> tuple[TaskLedger, str, str]:
    ...
    for target in (TaskStatus.TRIAGED, TaskStatus.PLANNED, TaskStatus.ASSIGNED, TaskStatus.IN_PROGRESS):
        cos.advance(task.task_id, target)
    approval = ApprovalService(ledger).request(
        task.task_id, "cos", "michael", AuthorityLevel.L4,
...
def _reply(text: str, *, envelope_id: str = "env-reply-001", event_id: str = "Ev-reply-001") -> dict:
    return {"envelope_id": envelope_id, "type": "events_api", "payload": {"type": "event_callback", "api_app_id": APP_ID, "event_id": event_id, "event": {"type": "message", "channel": CHANNEL_ID, "user": APPROVER_USER_ID, "text": text, "thread_ts": ROOT_TS, "ts": "1787843300.046169", "event_ts": "1787843300.046169"}}}


def _button(action_id: str, approval_id: str, *, envelope_id: str = "env-action-001") -> dict:
    return {"envelope_id": envelope_id, "type": "interactive", "payload": {"type": "block_actions", "api_app_id": APP_ID, "user": {"id": APPROVER_USER_ID}, "channel": {"id": CHANNEL_ID}, "container": {"type": "message", "channel_id": CHANNEL_ID, "message_ts": ROOT_TS}, "actions": [{"action_id": action_id, "value": approval_id}]}}
...
@pytest.mark.parametrize(("action_id", "disposition"), [("mesh_approval_approve", "APPROVE"), ("mesh_approval_deny", "DENY")])
...
def test_change_button_prompts_then_freeform_reply_becomes_governed_change_request() -> None:
    ...
    change = service.handle_envelope(_reply(instruction, envelope_id="env-change-text", event_id="Ev-change-text"))
    ...
    assert "SUPERSEDED_BY_CHANGE" in ledger.get_record("approval", approval_id)["decision_reason"]
...
def test_unbound_wrong_route_wrong_app_and_wrong_button_value_fail_closed() -> None:
    ...
    wrong_app = _reply("APPROVE"); wrong_app["payload"]["api_app_id"] = "A0OTHER"
