...
def test_v490_is_historical_and_v491_is_the_only_current_publisher() -> None:
    historical = _active_yaml_text(WORKFLOWS / "release-v4.9.0.yml")
    ...
    current = _active_yaml_text(WORKFLOWS / "release-v4.9.1.yml")
    assert "branches: [main]" in current
    assert "pull_request:" in current
    assert "gh release create v4.9.1" in current
    assert "--target \"$GITHUB_SHA\"" in current
    assert "contents: write" in current


def test_v491_release_identity_preserves_runtime_and_topology() -> None:
    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    release = (ROOT / "RELEASE.md").read_text(encoding="utf-8")
    docs_index = (ROOT / "docs/README.md").read_text(encoding="utf-8")
    for text in (readme, release, docs_index):
        assert "v4.9.1 Media OS Capability Registration" in text
