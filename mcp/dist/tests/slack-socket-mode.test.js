...
import { SlackSocketModeApprovalListener, readSlackSocketAppToken, } from '../slack-socket-mode.js';
class FakeSocket {
    onopen = null;
    onmessage = null;
    onerror = null;
...
function messageEnvelope(id = 'env-1') {
    return {
        envelope_id: id,
        type: 'events_api',
        payload: {
...
function interactiveEnvelope(id = 'env-action') {
    return {
        envelope_id: id,
        type: 'interactive',
        payload: {
...
test('structured authorization rejection is acknowledged without exposing internals', async () => {
    ...
    const tokenFile = tempSecret('xapp-test-token');
    const listener = new SlackSocketModeApprovalListener(env(tokenFile), {
        fetchImpl: async () => ({ ok: true, json: async () => ({ ok: true, url: 'wss://socket.test/link' }) }),
        socketFactory: () => socket,
...
test('required Socket Mode configuration errors remain fatal before background connection', async () => {
    const listener = new SlackSocketModeApprovalListener({ MESH_COS_SLACK_HITL_REQUIRED: 'true' }, {
...
test('Socket Mode is inert when HITL is not required', async () => {
    const listener = new SlackSocketModeApprovalListener({ MESH_COS_SLACK_HITL_REQUIRED: 'false' }, {
...
//# sourceMappingURL=slack-socket-mode.test.js.map