...
def _pending(*,action:str|None=None)->tuple[TaskLedger,str,str]:
 ...
 approval=ApprovalService(ledger).request(task.task_id,"cos","michael",AuthorityLevel.L4,action or f"Execute exact payload_fingerprint={FINGERPRINT}"); return ledger,task.task_id,approval.approval_id
def _config()->SlackSocketApprovalConfig:return SlackSocketApprovalConfig(channel_id=CHANNEL_ID,approver_user_id=APPROVER_USER_ID,approver_principal="michael",app_id=APP_ID)
def _notifier(ledger:TaskLedger)->SlackApprovalNotifier:
 def transport(method:str,payload:dict,token:str)->dict: assert token=="xoxb-test"; return {"ok":True,"channel":CHANNEL_ID,"ts":ROOT_TS if "thread_ts" not in payload else "1787843300.111111"}
...
def _bound(*,action:str|None=None)->tuple[TaskLedger,str,str,SlackSocketApprovalService]:
 ledger,task_id,approval_id=_pending(action=action); notifier=_notifier(ledger); notifier.post_approval(approval_id); return ledger,task_id,approval_id,SlackSocketApprovalService(ledger,_config(),notifier=notifier)
def _reply(text:str,*,envelope_id:str="env-reply",event_id:str="Ev-reply",thread_ts:str=ROOT_TS,ts:str="1787843300.046169")->dict:return {"envelope_id":envelope_id,"type":"events_api","payload":{"type":"event_callback","api_app_id":APP_ID,"event_id":event_id,"event":{"type":"message","channel":CHANNEL_ID,"user":APPROVER_USER_ID,"text":text,"thread_ts":thread_ts,"ts":ts,"event_ts":ts}}}
def test_root_or_unthreaded_message_is_non_authoritative()->None:
 ledger,_,approval_id,service=_bound(); root=_reply("APPROVE"); root["payload"]["event"].pop("thread_ts"); assert service.handle_envelope(root)["status"]=="IGNORED"; assert ledger.get_record("approval",approval_id)["status"]=="PENDING"
def test_manual_thread_approve_records_canonical_human_approval_idempotently()->None:
 ledger,task_id,approval_id,service=_bound(); decision=service.handle_envelope(_reply("approve")); assert service.handle_envelope(_reply("APPROVE"))==decision; assert decision["version"]=="mesh.cos.slack-human-decision.v5"; assert decision["source"]=="SLACK_SOCKET_MODE_HUMAN_INTERACTION"; assert decision["disposition"]=="APPROVE"; assert decision["canonical_principal"]=="michael"; assert decision["provider_identity_verified"] is True; assert decision["payload_fingerprint"]==FINGERPRINT; assert APPROVER_USER_ID not in str(decision); assert ledger.get_record("approval",approval_id)["status"]=="APPROVED"; assert ledger.get_task(task_id).status==TaskStatus.READY_FOR_ACTION
...
def test_config_reads_protected_approver_identity_file_without_slash_command(tmp_path:Path)->None:
 approver_file=tmp_path/"approver-id"; approver_file.write_text(APPROVER_USER_ID+"\n",encoding="utf-8"); assert SlackSocketApprovalConfig.from_env({"MESH_COS_SLACK_AGENT_OPS_CHANNEL_ID":CHANNEL_ID,"MESH_COS_SLACK_APPROVER_USER_ID_FILE":str(approver_file),"MESH_COS_SLACK_APPROVER_PRINCIPAL":"michael","MESH_COS_SLACK_APP_ID":APP_ID})==_config()
...
def test_provider_envelope_validation_is_strict_and_bounded()->None:
 _,_,_,service=_bound(); cases=[]; wrong_type=_reply("APPROVE"); wrong_type["type"]="slash_commands"; cases.append((wrong_type,"events_api or interactive")); missing_envelope=_reply("APPROVE"); missing_envelope.pop("envelope_id"); cases.append((missing_envelope,"envelope_id")); invalid_payload=_reply("APPROVE"); invalid_payload["payload"]="invalid"; cases.append((invalid_payload,"payload is invalid")); wrong_callback=_reply("APPROVE"); wrong_callback["payload"]["type"]="url_verification"; cases.append((wrong_callback,"event_callback")); missing_event_id=_reply("APPROVE"); missing_event_id["payload"].pop("event_id"); cases.append((missing_event_id,"event_id")); invalid_event=_reply("APPROVE"); invalid_event["payload"]["event"]="invalid"; cases.append((invalid_event,"event payload is invalid")); wrong_event_type=_reply("APPROVE"); wrong_event_type["payload"]["event"]["type"]="reaction_added"; cases.append((wrong_event_type,"message events only")); missing_channel=_reply("APPROVE"); missing_channel["payload"]["event"].pop("channel"); cases.append((missing_channel,"channel"))
...
def test_bot_binding_requires_pending_michael_owned_fingerprinted_approval()->None:
 ...
 with pytest.raises(ValueError,match="PENDING"):_notifier(ledger3).post_approval(approval_id3)
