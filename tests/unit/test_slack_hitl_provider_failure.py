...
def _setup() -> tuple[TaskLedger, SlackSocketApprovalService, str]:
    ledger = TaskLedger()
    cos = ChiefOfStaffService(ledger)
    task = cos.intake(
        objective="Synthetic Slack provider failure acceptance",
        expected_outcome="Unbound or app-authored Slack input cannot authorize action",
        requested_by="cos",
        executive_sponsor="michael",
        accountable_agent="cos",
        decision_owner="michael",
        authority_level=AuthorityLevel.L4,
        acceptance_test="provider event must be a manual human reply in the bound approval thread",
...
def _event(text: str, *, thread_ts: str | None = None, app_id: str | None = None) -> dict:
    ...
    if app_id is not None:
        event["app_id"] = app_id
    return {
        "envelope_id": "env-provider",
        "type": "events_api",
        "payload": {
            "type": "event_callback",
            "event_id": "Ev-provider",
            "event": event,
        },
    }
...
def test_unbound_provider_message_never_becomes_approval_evidence() -> None:
    ...
    with pytest.raises(PermissionError, match="bound"):
        service.handle_envelope(_event("APPROVE", thread_ts=ROOT_TS))

    assert ledger.get_record("approval", approval_id)["status"] == "PENDING"
    assert ledger.get_record("approval_slack_socket_decision", approval_id) is None


def test_app_authored_provider_message_never_becomes_human_approval_evidence() -> None:
    ledger, service, approval_id = _setup()
    service.handle_envelope(
        _event(f"Approval ID: `{approval_id}`", app_id="A0CHATGPT")
    )

    with pytest.raises(PermissionError, match="app-authored"):
        service.handle_envelope(
            _event("APPROVE", thread_ts="1787843300.046169", app_id="A0CHATGPT")
        )
