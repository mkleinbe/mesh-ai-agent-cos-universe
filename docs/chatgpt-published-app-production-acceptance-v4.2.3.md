# ChatGPT / Slack Production Acceptance v4.2.3

Status: **REQUIRED AFTER DEPLOYMENT, BEFORE CONSEQUENTIAL HITL USE**

## Preconditions

- QNAP runtime is deployed from the immutable v4.2.3 release bundle.
- `mesh-cos-mcp` and `mesh-cos-tunnel` are healthy.
- runtime reports `mcp_version=4.0.0`, `deployment_release=4.2.3`, `slack_hitl_mode=CHATGPT_NATIVE_EVENT_TRIGGER`, and `slack_hitl_ready=true`.
- QNAP deployment verification passed the live Slack provider-read gate from the running container after bounded qnet egress-readiness handling.
- the installed dedicated Slack bot has `chat:write` and `groups:history`, is a member of `#mesh-agent-ops`, and uses App ID `A0B49RNE4K0`.
...
The authority flow is unchanged from v4.2.2.

## Priority incident replay

The first live case must close the production defects discovered during v4.2.0 through v4.2.2 acceptance:
...
| Provider auth/scope failure | Slack provider read returns `ok:false` | immediate fail-closed; no canonical mutation |
| Fresh qnet network not ready | provider fetch raises network exception | bounded deployment retry only; no authority mutation |
| Slack unavailable beyond retry window | no provider response after all attempts | deployment verification fails and rolls back |

## Provider-read deployment gate

Before the first approval, QNAP deployment verification must call `conversations.history` from the running `mesh-cos-mcp` container using the mounted bot token and governed channel. This is not approval evidence; it is deployment evidence that the actual runtime credential and fresh qnet namespace can reach and read the private channel.

The v4.2.3 verifier permits up to six attempts with five-second delay only when `fetch()` raises a network exception before any provider response is received. A Slack `ok:false` response, malformed response, or internal verifier failure terminates immediately. If the bounded network window is exhausted, deployment fails and transactional rollback remains mandatory.

## Evidence to retain

For each live case retain, without secret values: deployment release, merge SHA and image identity; MCP runtime contract version; Work dispatcher run result; Slack channel ID, root thread timestamp and reply timestamp; canonical approval ID and task ID; provider reconciliation disposition/status; TaskLedger before/after state; replay evidence where applicable; sanitized provider error code for negative provider cases; and `governance.verify_audit_chain` result.

## Exit criteria

Production acceptance passes only when local v4.2.3 deployment verification passes, all positive cases create exactly the expected canonical state, all negative cases fail closed, duplicate delivery is idempotent, the observed wake-up is the ChatGPT Work event trigger, provider message text is independently reread by QNAP, no QNAP Slack WebSocket listener or xapp secret is active, Secure MCP Tunnel remains the sole remote MCP ingress, and the final TaskLedger audit chain verifies.
