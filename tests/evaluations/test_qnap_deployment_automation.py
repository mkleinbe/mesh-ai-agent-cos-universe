...
def test_prepare_automates_configuration_without_leaking_secret() -> None:
    ...
    assert "mesh-cos-mcp:qnap-v4.1.9" in prepare
...
def test_release_bundle_contains_v419_closeout_docs_helpers_metadata_and_no_runtime_secret() -> None:
    ...
    assert "qnap-security-review-v4.1.9.md" in builder
    assert "release-4.1.9-documentation-closeout.md" in builder
    assert "chatgpt-published-app-production-acceptance-v4.1.9.md" in builder
    assert "qnap-release-closeout-v4.1.9.feature" in builder
...
def test_deployment_steps_contain_v419_sudo_and_log_receipt() -> None:
    steps = text(QNAP / "DEPLOYMENT-STEPS.md")
    assert "v4.1.9" in steps
...
def test_v419_release_closeout_bdd_is_ready() -> None:
    feature = text(ROOT / "specs" / "qnap-release-closeout-v4.1.9.feature")
    assert "@ready" in feature
    for scenario_id in ["QNAP-069", "QNAP-070", "QNAP-071", "QNAP-072", "QNAP-073"]:
        assert f"Scenario: {scenario_id}" in feature
    for token in ["4.1.9", "exactly 10", "exactly 27", "COMPLETED", "VERIFIED", "OpenAI Secure MCP Tunnel"]:
        assert token in feature
