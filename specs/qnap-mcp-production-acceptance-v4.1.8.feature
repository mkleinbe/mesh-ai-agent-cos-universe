...
Feature: Mesh CoS MCP v4.1.8 production acceptance remediation
  ...
  Scenario: QNAP-059 Published tool schemas match runtime request validation
  ...
  Scenario: QNAP-060 Invalid structured input returns bounded field-level guidance
  ...
  Scenario: QNAP-061 Canonical task identifiers resolve consistently across task operations
  ...
  Scenario: QNAP-062 Declared governed Skills resolve through server registration and unauthorized Skills fail closed
    Given a Skill declared for an agent in the canonical registry
    When that agent invokes skills.invoke_governed
    Then the server returns an auditable CHATGPT_SKILL_HANDOFF record with agent and capability provenance
  ...
  Scenario: QNAP-063 AgentOps recommendation uses the published structured request contract
  ...
  Scenario Outline: QNAP-064 Every Phase 1 agent runtime is immutably bound to its own identity
  ...
  Scenario: QNAP-065 Multi-agent delegation preserves direct-child and terminal boundaries
  ...
  Scenario: QNAP-066 Completion never implies verification
  ...
  @QNAP-067 @audit
  Scenario: QNAP-067 Remediation preserves tamper-evident governance audit integrity
  ...
  Scenario: QNAP-068 Packaged and hosted v4.1.8 behavior agree
