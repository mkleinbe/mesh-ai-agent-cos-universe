...
## Durable security receipt

This receipt verifies the security properties of the v4.8.1 release-control design. It intentionally does not carry a mutable publication-pending flag. The authoritative post-publication identity is the GitHub Release target created by `.github/workflows/release-v4.8.1.yml`, which is required to equal the merged `GITHUB_SHA`.

The release-control candidate was independently verified through canonical and targeted CI before this durable receipt normalization. A final exact-head recheck is required after the receipt, workflow assertion, and regression are updated together.
...
7. Tagged source -> external GitHub publication state.
...
- Regression fixtures may pin independently verified candidate evidence and run IDs, but must not require a self-referential final evidence commit SHA.
- Tagged source must not contain a mutable `PENDING` publication-state marker that necessarily becomes stale after successful release.

## Verification evidence

Exact-head canonical CI run `34724550131` passed the full repository verification surface on `31063070a5794c40eb76fc3a27138c87e60740e3`, including contract/drift/package/action checks, Ruff, Mypy, 100% core coverage, Bandit, QNAP regressions, production-equivalent container build, and MCP discovery/sequential-request verification.

Exact-head targeted run `34724552349` passed the v4.8.1 release-state contract on the same candidate, including exact-SHA publishing configuration, historical-publisher retirement, candidate verification receipt checks, receipt path gating, and documentation-only runtime/security boundaries.

Earlier independent verification on release-control candidate `f1fa3601e373515950e61ead7c6b9cbdb37fdb28` also passed canonical run `34724369995` and targeted run `34724369989`.
...
- `SEC-481-04` The v4.8.1 verification receipt was initially absent from the workflow path filters, allowing an evidence-only receipt change to bypass the targeted gate: **RESOLVED** by adding the receipt to push/PR filters, checking its PASS marker, and enforcing the path in `test_release_state_v481.py`.
- `SEC-481-05` The first evidence-only recheck found that the regression still required an obsolete intermediate candidate SHA after the receipt advanced to independently verified evidence: **RESOLVED** by pinning the regression to independently verified candidate evidence and successful run IDs. No runtime or security acceptance criterion was weakened.
- `SEC-481-06` A mutable `PENDING` publication flag in the v4.8.1 tagged source would recreate the stale-release-document defect and force a later PATCH merely to flip the flag: **RESOLVED** by separating immutable source verification from externally authoritative release identity and enforcing exact-SHA publication in the workflow.

No critical or high security finding remains open in the v4.8.1 release-state design.

## Release-control result

The design satisfies the required security properties:
...
- tagged source uses durable PASS semantics rather than mutable publication state;
...
The existing MCP dependency graph reports one moderate Hono advisory during `npm audit`. This condition predates v4.8.1 and is not introduced or modified by this docs/release-control PATCH. Existing canonical security checks remain green under the repository's current policy. Remediation belongs in a separately scoped dependency/security maintenance change.

## Final security gate

Security status is **PASS** for the bounded v4.8.1 design. A final exact-head canonical and targeted recheck must pass after this durable-receipt normalization. After merge, final publication proof is the externally observable equality of `main`, tag `v4.8.1`, and the GitHub Release target together with successful main-branch release workflow execution.
