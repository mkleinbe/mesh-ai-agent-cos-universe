...
No Slack `xapp-` Socket Mode credential is required or permitted by the production v4.4.1 HITL path.

## v4.4.1 release-root staging

- [ ] `mesh-cos-mcp-qnap-v4.4.1.zip` and `.sha256` are placed directly in `/share/Docker/cos-mcp/releases`
- [ ] operator working directory is `/share/Docker/cos-mcp/releases`
- [ ] `sha256sum -c mesh-cos-mcp-qnap-v4.4.1.zip.sha256` passes
...
- [ ] `v4.4.1/cos-mcp/release-metadata.txt` reports `version=4.4.1` and the exact authorized commit
- [ ] release-directory basename agrees with staged metadata
- [ ] build context contains the v4.4.1 MCP contract with `delegation.execute_owner`
...
- [ ] active release/image are `4.4.1` / `mesh-cos-mcp:qnap-v4.4.1-<source-sha12>`
...
- [ ] `docs/chatgpt-published-app-production-acceptance-qnap-v4.4.1.md` passes before recovery
