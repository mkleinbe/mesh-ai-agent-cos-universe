...
def test_qnap_release_preserves_v4_authority_contract() -> None:
    ...
    assert "TAG: v4.1.6" in release_workflow
    assert '--title "v4.1.6 Secure MCP Published App Production Identity"' in release_workflow
    release_notes = (ROOT / "RELEASE.md").read_text()
    assert "v4.1.6 Secure MCP Published App Production Identity" in release_notes
