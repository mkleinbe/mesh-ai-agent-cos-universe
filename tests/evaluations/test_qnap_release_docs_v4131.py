...
def test_active_qnap_operator_docs_are_aligned_to_v440() -> None:
    for relative in ACTIVE_QNAP_DOCS:
        text = (ROOT / relative).read_text(encoding="utf-8")
        assert "4.3.0" not in text, relative
        assert "v4.3.0" not in text, relative
        assert "4.4.0" in text, relative


def test_current_qnap_commands_reference_exact_v440_asset_and_release_dir() -> None:
    steps = (ROOT / "deployment/qnap/DEPLOYMENT-STEPS.md").read_text(encoding="utf-8")
    assert "mesh-cos-mcp-qnap-v4.4.0.zip" in steps
    assert "mesh-cos-mcp-qnap-v4.4.0.zip.sha256" in steps
    assert "sudo sh ./v4.4.0/mesh-cos-mcp-deploy.sh" in steps

    env = (ROOT / "deployment/qnap/.env.example").read_text(encoding="utf-8")
    assert "MESH_COS_DEPLOYMENT_RELEASE=4.4.0" in env
    assert "MESH_COS_IMAGE=mesh-cos-mcp:qnap-v4.4.0" in env
