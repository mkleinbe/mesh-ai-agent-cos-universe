...
QNAP_BUNDLE_APP_ROOT="$BUNDLE"; QNAP_APP_ROOT="$APP"; QNAP_SLACK_APPROVER_USER_ID_FILE="$APP/secrets/slack-approver-user-id"; QNAP_SLACK_SOCKET_APP_TOKEN_FILE="$APP/secrets/slack-socket-app-token"; QNAP_SLACK_BOT_TOKEN_FILE="$APP/secrets/slack-bot-token"; MESH_COS_LOG_ROOT="$APP/logs/deployment"; MESH_UID=65532; MESH_GID=65532
export QNAP_SCRIPT_ROOT QNAP_BUNDLE_APP_ROOT QNAP_APP_ROOT QNAP_SLACK_APPROVER_USER_ID_FILE QNAP_SLACK_SOCKET_APP_TOKEN_FILE QNAP_SLACK_BOT_TOKEN_FILE MESH_COS_LOG_ROOT MESH_UID MESH_GID
. "$QNAP_SCRIPT_ROOT/mesh-cos-slack-hitl-configure.sh"
[ -s "$APP/secrets/slack-approver-user-id" ]; [ "$(cat "$APP/secrets/slack-approver-user-id")" = 'U01KG3CNYHK' ] || exit 1; [ -s "$APP/secrets/slack-socket-app-token" ]; [ -s "$APP/secrets/slack-bot-token" ]
grep -q -- 'image inspect image:test' "$CALLS"; grep -q -- 'slack-approver-user-id' "$CALLS"; grep -q -- 'slack-socket-app-token' "$CALLS"; grep -q -- 'slack-bot-token' "$CALLS"
...
test -f "$PROVISION"; test -f "$SECRET_INPUT"; test -f "$TUNNEL_PROVISION"; grep -q 'mesh_read_secret_tty' "$PROVISION"; grep -q 'Slack Socket Mode app-level token (input hidden)' "$PROVISION"; grep -q 'Slack bot OAuth token (input hidden)' "$PROVISION"; grep -q 'xoxb-' "$PROVISION"; ! grep -q 'slack-verifier-token' "$PROVISION"; grep -q 'mesh_shell_supports_silent_read' "$SECRET_INPUT"; grep -q '/bin/stty /usr/bin/stty' "$SECRET_INPUT"; ! grep -q 'read_secret_tty' "$PREPARE"; ! grep -q 'QNAP_SLACK_VERIFIER_TOKEN_FILE' "$PREPARE"; grep -q 'mesh-cos-tunnel-key-provision.sh' "$PREPARE"
LOGS=$(find "$APP/logs/deployment" -type f -print); [ -n "$LOGS" ]
if grep -R -q 'U01KG3CNYHK\|xoxb-test-bot\|xapp-test-socket\|tunnel-test-value' "$APP/logs/deployment"; then echo 'FAIL Slack HITL protected value leaked to deployment logs' >&2; exit 1; fi
echo 'PASS Slack HITL deploy path validates governed approver, Socket Mode, and dedicated bot credentials without logging protected values'
