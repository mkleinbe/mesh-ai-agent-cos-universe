...
v4.1.17 replaces the failed slash-command HITL path with the dedicated **ChatGPT Enterprise AI Agent** Slack bot, Block Kit buttons, and authenticated thread replies. The canonical Phase 1 MCP runtime contract remains **4.0.0**.

## Canonical paths

- operator release root: `/share/Docker/cos-mcp/releases`
- extracted release: `/share/Docker/cos-mcp/releases/v4.1.17`
...
Stay in `/share/Docker/cos-mcp/releases`. The ZIP creates `v4.1.17/` during extraction.

## Slack prerequisite

Apply `slack-app-manifest.v4.1.17.json` to Slack, reinstall/re-authorize the app for the required private-channel history scope, enable Socket Mode/interactivity, and ensure **ChatGPT Enterprise AI Agent** is a member of `mesh-agent-ops`.
...
If Slack credentials are missing:

```sh
sudo sh ./v4.1.17/mesh-cos-slack-hitl-provision.sh
```

If the OpenAI tunnel runtime key is missing:

```sh
sudo sh ./v4.1.17/mesh-cos-tunnel-key-provision.sh
```

## Safe v4.1.17 deployment

Place `mesh-cos-mcp-qnap-v4.1.17.zip` and `mesh-cos-mcp-qnap-v4.1.17.zip.sha256` directly in `/share/Docker/cos-mcp/releases`, then run:

```sh
cd /share/Docker/cos-mcp/releases
sha256sum -c mesh-cos-mcp-qnap-v4.1.17.zip.sha256
unzip -oq mesh-cos-mcp-qnap-v4.1.17.zip
sudo sh ./v4.1.17/mesh-cos-mcp-deploy.sh
```

If deployment reports missing Slack credentials:
...
```sh
cd /share/Docker/cos-mcp/releases
sudo sh ./v4.1.17/mesh-cos-mcp-backup.sh manual
sudo sh ./v4.1.17/mesh-cos-mcp-preflight.sh
sudo sh ./v4.1.17/mesh-cos-mcp-verify.sh
```
...
PASS requires active release `4.1.17`, application image `mesh-cos-mcp:qnap-v4.1.17`, healthy application/tunnel containers, `slack_hitl_ready=true`, and:
...
After local deployment passes, run `CHATGPT-ACCEPTANCE.md` and `chatgpt-published-app-production-acceptance-v4.1.17.md`. Production acceptance requires the actual QNAP serving instance and live Slack HITL checks.
