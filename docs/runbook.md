...
3. Confirm the QNAP deployment train is `4.1.6` in `Dockerfile`, `.env.example`, prepare defaults, release bundle, CI, and release workflow.
...
Repository CI additionally builds and exercises the production image, renders QNAP Compose, proves dual release identity, runs modern MCP discovery and sequential calls, validates direct-ingress denial, checks non-root/read-only/no-capability/no-Docker-socket controls, and verifies SQLite backup/restart recovery.

## QNAP deployment path

1. Place `mesh-cos-mcp-qnap-v4.1.6.zip` and its `.sha256` receipt in `/share/Docker`.
...
4. Require pre-deploy backup, release-metadata/environment equality, image identity checks, Compose rendering, container health, runtime preflight, ingress denial, and post-deploy backup.
5. Verify `/healthz` and `/readyz` report:
...
The remote process must fail closed if `MESH_COS_DEPLOYMENT_RELEASE` is missing or blank.
...
4. Run the ten-call sequential read-only acceptance in `deployment/qnap/CHATGPT-ACCEPTANCE.md` without restarting either container.
...
7. Require every successful governed tool envelope to report `mcp_version=4.0.0`, `deployment_release=4.1.6`, and `agent_id=cos`.
...
- deployment-release metadata does not affect tool selection or authority.
...
If the kill switch, ledger integrity, audit chain, identity binding, source authority, approval evidence, MCP package, deployment identity, or Secure MCP Tunnel ingress boundary is invalid, stop consequential execution. Do not improvise around a failed gate.
...
If hosted calls regress to HTTP 502 or `invalid_session`, capture the QNAP diagnostic log and container health first. Do not rotate tunnel credentials, delete TaskLedger state, or rebuild the environment until the transport boundary is isolated.
