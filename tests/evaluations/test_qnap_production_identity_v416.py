...
def test_active_deployment_train_is_v442_while_historical_v430_wrapper_is_preserved() -> None:
    ...
    assert "MESH_COS_DEPLOYMENT_RELEASE=4.4.2" in env_example
    ...
    assert "Build current-source v4.4.2 QNAP candidate artifacts" in ci
    assert "QNAP_VERSION: '4.4.2'" in ci
    assert 'bash scripts/build-qnap-release-bundle.sh "$QNAP_VERSION"' in ci
    assert "dist/mesh-cos-mcp-qnap-v4.4.2.zip" in ci
...
def test_chatgpt_acceptance_requires_owner_execution_after_v442_deploy() -> None:
