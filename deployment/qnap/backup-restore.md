...
`mesh-cos-mcp-deploy.sh` automatically creates a pre-deploy backup when an existing service is running and a post-deploy backup after verification succeeds. A manual backup can be run from `/share/Docker`:
...
5. Restore `taskledger.sqlite3` to `/share/Docker/cos-mcp/state/ledger/taskledger.sqlite3` and restore ownership `65532:65532`, mode `0660` through the governed deployment/permission path rather than weakening the state mode.
...
8. Run `cd /share/Docker && sudo sh mesh-cos-mcp-preflight.sh`.
9. Start/deploy through `sudo sh mesh-cos-mcp-deploy.sh`; require automated verification and a new post-restore backup.
