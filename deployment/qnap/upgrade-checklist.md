...
- [ ] Exact v4.4.2 candidate passes Python, Node, contract/doc drift, owner-readiness, security, QNAP shell, bundle, container provenance, and modern MCP gates
...
- [ ] security receipt is bound to the exact v4.4.2 candidate SHA
- [ ] independent verification receipt is bound to the exact v4.4.2 candidate SHA
- [ ] SQLite schema compatibility evaluated; no destructive migration required
- [ ] approved v4.4.2 ZIP and checksum are placed directly in `/share/Docker/cos-mcp/releases`
...
- [ ] operator uses `sudo sh ./v4.4.2/mesh-cos-mcp-deploy.sh`
- [ ] deployment validates `v4.4.2` beneath the canonical releases root and matches staged metadata
...
- [ ] staged `release-metadata.txt` reports `version=4.4.2` and exact release commit
...
- [ ] run `docs/chatgpt-published-app-production-acceptance-qnap-v4.4.2.md`
