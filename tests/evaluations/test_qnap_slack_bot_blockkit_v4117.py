...
def test_qnap_122_dedicated_bot_is_canonical_outbound_slack_path() -> None:
    ...
    assert "icon_emoji" not in source
    assert "icon_url" not in source
...
def test_qnap_123_124_socket_mode_dispatches_block_actions_and_change_prompt() -> None:
    socket = read("mcp/src/slack-socket-mode.ts")
    service = read("src/mesh_cos/slack_socket_approval.py")
    assert "interactive" in socket
    assert "block_actions" in service
    assert "AWAITING_CHANGE_INPUT" in service
    assert "What would you like to change?" in service
    assert "post_thread_reply" in service
...
def test_qnap_127_manifest_matches_private_channel_bot_surface() -> None:
    manifest = json.loads(read("deployment/qnap/slack-app-manifest.v4.1.17.json"))
    ...
    assert {"chat:write", "groups:history"}.issubset(scopes)
...
def test_qnap_120_bot_token_is_protected_and_slash_command_is_absent() -> None:
    ...
    provision = read("deployment/qnap/scripts/mesh-cos-slack-hitl-provision.sh")
    ...
    assert "MESH_COS_SLACK_APPROVAL_COMMAND" not in preflight
    assert "Slack bot OAuth token" in provision
