...
def test_qnap_release_preserves_v4_authority_contract() -> None:
    ...
    current_release_workflow = (
        ROOT / ".github" / "workflows" / "release-v4.1.16.yml"
    ).read_text()
    assert "v4.1.16 QNAP release candidate" in current_release_workflow
    assert "gh release create v4.1.16" in current_release_workflow
    historical_v414 = (
    ...
    assert "v4.1.16 QNAP Restarting-Runtime Backup Hotfix" in release_notes
