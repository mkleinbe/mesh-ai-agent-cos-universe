...
def _slack_error_code(decoded: object) -> str:
    if isinstance(decoded, Mapping):
        value = str(decoded.get("error") or "").strip().lower()
        if _SAFE_SLACK_ERROR_RE.fullmatch(value):
            return value
...
def _default_transport(method: str, payload: dict[str, Any], token: str) -> dict[str, Any]:
    ...
    if method in _SLACK_GET_METHODS:
        query = urllib.parse.urlencode(_query_payload(payload))
    ...
    if not isinstance(decoded, dict) or decoded.get("ok") is not True:
        raise RuntimeError(
            f"Slack Web API rejected the request: {_slack_error_code(decoded)}"
        )
    return decoded
...
class SlackApprovalNotifier:
    ...
    def post_approval(self, approval_id: str) -> dict[str, Any]:
        ...
        if approval.get("approval_owner") != "michael":
            raise PermissionError("Canonical approval owner is not the governed human principal")
        ...
        binding = {
            "version": "mesh.cos.slack-thread-binding.v3",
    ...
    def mark_change_request_revised(
        self,
