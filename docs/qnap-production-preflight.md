...
The v4.1.4 transport correction remains unchanged. Current releases replace the legacy server-managed MCP session lifecycle that caused valid modern `server/discover` requests to fail with `invalid_session`. Production readiness requires the current MCP discovery path to succeed, not only Python/TaskLedger health.
