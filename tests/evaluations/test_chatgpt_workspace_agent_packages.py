...
def test_all_phase1_agents_have_chatgpt_skill_and_builder_config() -> None:
    ...
    assert (AGENTS / "message-ops.json").exists()
    assert (ROOT / "agents" / "message-ops.md").exists()
...
def test_external_shared_capabilities_are_explicit_and_bounded() -> None:
    ...
    assert set(capabilities) == {"mesh-devils-advocate", "mesh-data-analytics", "mesh-opex-bot"}
...
def test_builder_configs_have_least_privilege_tool_allowlists() -> None:
    ...
    assert "message-ops" in contract_allowlists
...
def test_mcp_contract_and_server_side_policy_fail_closed() -> None:
    ...
    assert policy.authorize("message-ops", "approval.get")["name"] == "approval.get"
...
def test_risky_app_and_message_execution_boundaries_remain_fail_closed() -> None:
    ...
    assert message_ops.get("shared_skills", []) == []
    assert any("exact approved recipients" in rule for rule in message_ops["connector_action_constraints"])
    assert any("approval reference" in rule for rule in message_ops["connector_action_constraints"])
    assert message_ops["write_action_policy"]["default"] == "ALWAYS_ASK"
...
def test_workspace_agent_builder_handoff_prompt_is_exact_and_complete() -> None:
    ...
    roster = text.split("Create exactly these", 1)[-1].split("Do **not** create", 1)[0]
    assert "Message Operations" in roster
    assert "Devil's Advocate" not in roster
