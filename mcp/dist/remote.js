...
import { createServer, loadContract, requireAgentId, requireDeploymentRelease } from './server.js';
const NATIVE_SLACK_MODE = 'CHATGPT_NATIVE_EVENT_TRIGGER';
function ip(req) { return (req.socket.remoteAddress ?? '').replace(/^::ffff:/, ''); }
function allowed(req, env) { const expected = env.MCP_TRUSTED_CLIENT_IP?.trim(); return Boolean(expected) && ip(req) === expected; }
function json(res, status, value) { const out = JSON.stringify(value); res.writeHead(status, { 'content-type': 'application/json', 'content-length': Buffer.byteLength(out) }); res.end(out); }
function protectedFilePresent(pathValue) {
...
export function nativeSlackHitlConfigured(env = process.env) {
...
function statusPayload(ok, contract, agentId, deploymentRelease, slackHitlReady) {
...
async function runtimeCoreReady(env) {
    ...
    if (record?.status !== 'ACTIVE')
        throw new Error('bound_agent_not_active');
...
async function runtimeReady(env) {
    ...
    if (!nativeSlackHitlConfigured(env))
        throw new Error('native_slack_hitl_not_configured');
}
function discoveryRequest(deploymentRelease) {
...
async function protocolReady(handler, deploymentRelease) {
    const response = await handler.fetch(discoveryRequest(deploymentRelease));
    const text = await response.text();
    if (response.status !== 200 || !text.includes('2026-07-28'))
        throw new Error('modern_mcp_discovery_unavailable');
}
export async function startRemote(env = process.env) {
    if ((env.MCP_AUTH_MODE ?? '').trim() !== 'tunnel')
        throw new Error('Remote MCP requires MCP_AUTH_MODE=tunnel; controlled HTTPS OAuth is not enabled by this deployment');
    if (!env.MCP_TRUSTED_CLIENT_IP?.trim())
        throw new Error('MCP_TRUSTED_CLIENT_IP is required');
    const contract = loadContract();
    ...
    const app = http.createServer(async (req, res) => {
        try {
            ...
            if (url.pathname === '/healthz')
                return json(res, 200, statusPayload(true, contract, agentId, deploymentRelease, slackReady));
            ...
            if (!res.headersSent)
                json(res, 500, { error: 'mcp_transport_error' });
            else
                res.end();
        }
    });
