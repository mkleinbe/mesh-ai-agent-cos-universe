...
class MetricsService:
    ...
    def summary(self) -> dict:
        ...
        completed = [task for task in tasks if task.status in {TaskStatus.COMPLETED, TaskStatus.VERIFIED, TaskStatus.CLOSED}]
        ...
        time_avoided = sum(int(task.ceo_time_avoided_estimate_minutes or 0) for task in verified if task.ceo_time_avoided_methodology)
        ceo_touches_per_completed = sum(task.CEO_touches for task in completed) / len(completed) if completed else 0.0
        ...
        cycle_times = [duration for task in verified if (duration := self._duration_minutes(task.started_at or task.created_at, task.verified_at or task.closed_at)) is not None]
        ...
        approval_times = [duration for record in approvals if (duration := self._duration_minutes(record.get("requested_at"), record.get("decided_at"))) is not None]
        ...
        deflected = [record for record in answer_records if record.get("disposition") in {"ANSWERED", "RECOMMENDATION_PROVIDED", "ROUTED"}]
        answer_deflection = len(deflected) / len(answer_records) if answer_records else 0.0
        answer_resolution_times = [duration for record in answer_records if (duration := self._duration_minutes(record.get("received_at"), record.get("resolved_at"))) is not None]
        ...
        return {
            ...
            "correct_escalation_rate": round(correct_escalations / escalation_total, 4) if escalation_total else 0.0,
            "false_escalation_rate": round(false_escalations / escalation_total, 4) if escalation_total else 0.0,
            "missed_escalation_rate": round(missed_escalations / escalation_total, 4) if escalation_total else 0.0,
            ...
            "answer_desk_incorrect_answers": sum(1 for record in answer_records if record.get("incorrect")),
            "answer_desk_corrected_answers": sum(1 for record in answer_records if record.get("corrected")),
            "answer_desk_access_control_failures": sum(1 for record in answer_records if record.get("access_control_failure")),
            "answer_desk_median_resolution_minutes": round(median(answer_resolution_times), 3) if answer_resolution_times else 0.0,
        }
