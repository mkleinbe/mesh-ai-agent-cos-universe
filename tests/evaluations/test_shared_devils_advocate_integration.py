...
def test_qnap_release_preserves_v4_authority_contract() -> None:
    ...
    assert "TAG: v4.1.8" in release_workflow
    assert '--title "v4.1.8 MCP Contract Validation and Governed Skill Handoff"' in release_workflow
    release_notes = (ROOT / "RELEASE.md").read_text()
    assert "v4.1.8 MCP Contract Validation and Governed Skill Handoff" in release_notes
