...
No Slack `xapp-` Socket Mode credential is required or permitted by the production v4.4.2 HITL path.

## v4.4.2 release-root staging

- [ ] `mesh-cos-mcp-qnap-v4.4.2.zip` and `.sha256` are placed directly in `/share/Docker/cos-mcp/releases`
- [ ] operator working directory is `/share/Docker/cos-mcp/releases`
- [ ] `sha256sum -c mesh-cos-mcp-qnap-v4.4.2.zip.sha256` passes
- [ ] extraction creates `v4.4.2/` automatically
- [ ] no manual release-directory creation, helper copy, payload move, or chmod is required
- [ ] `v4.4.2/cos-mcp/release-metadata.txt` reports `version=4.4.2` and the exact authorized commit
- [ ] release-directory basename agrees with staged metadata
- [ ] build context contains the v4.4.2 MCP contract with `delegation.execute_owner`
...
- [ ] active release/image are `4.4.2` / `mesh-cos-mcp:qnap-v4.4.2-<source-sha12>`
- [ ] `/healthz` and `/readyz` report `mcp_version=4.0.0`, `deployment_release=4.4.2`, `agent_id=cos`, `transport=SECURE_MCP_TUNNEL`
...
- [ ] `docs/chatgpt-published-app-production-acceptance-qnap-v4.4.2.md` passes before recovery
